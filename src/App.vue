...
import Button from "./components/onboarding/Button.vue";
import StepColorPicker from "./components/onboarding/StepColorPicker.vue";
...
<template>
  ...
  <div v-else-if="onboardingLoaded && !onboardingCompleted" class="onboarding-shell" :style="hueVariables">
    <main class="onboarding-window" @mouseenter="setOnboardingPopupInteractive(true)"
      ...
      <Button type="button" variant="secondary" class="close-button" aria-label="Close application"
      ...
      <StepColorPicker v-else :hue="onboardingData.hue" :assigned-hue="onboardingData.assignedHue"
        :disabled="onboardingSaving" @manual-hue-change="onManualHueChange" @reset-hue="onResetHueToAssigned" />

      <div class="actions">
        <Button v-if="onboardingStep > 1" type="button" variant="secondary" :disabled="onboardingSaving"
        ...
        <Button v-if="onboardingStep < totalOnboardingSteps" type="button" variant="primary"
        ...
        <Button v-else type="button" variant="primary" :disabled="onboardingSaving" @click="completeOnboarding">
          <div></div>
          Finish
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="size-4">
            <path fill-rule="evenodd"
