...
const calculateAssignedProfile = ({ sliders, traits, hue }) => {
  ...
  */
  ...
  calculatedSymmetry = (1 - normalizedSliders.groundedCreative) * 0.5 + normalizedTraits.includes("chill") * 0.5;
  calculatedVariability = normalizedSliders.rationalEmotional * 0.5 + normalizedTraits.includes("mysterious") * 0.5;
  calculatedActivity = normalizedSliders.calmAssertive * 0.3 + (1 - normalizedTraits.includes("gentle")) * 0.2 + normalizedTraits.includes("active") * 0.5;
  calculatedReaction = normalizedTraits.includes("cute")
  ...
  <div v-else-if="onboardingLoaded && !onboardingCompleted" class="onboarding-shell" :style="hueVariables">
    <main class="onboarding-window" @mouseenter="setOnboardingPopupInteractive(true)"
      ...
      <section v-if="onboardingStep === 1">
        <h1>Meet your Self Companion</h1>
        <div class="boxes">
          <div class="box">
            <div class="box-left">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="size-4">
      ...
      <section v-else-if="onboardingStep === 2">
        ...
        <p class="description">Adjust the sliders and choose traits to reflect your personality. This will impact the
