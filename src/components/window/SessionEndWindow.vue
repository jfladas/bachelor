...
<template>
    <div class="overlay">
        <section class="window" role="dialog" aria-label="End of session" @pointerenter="emit('pointer-enter')"
            ...
            <h1 class="title">Print your blob sheet?</h1>
            <p class="description">{{ wrapUpText }}</p>
            <p class="description">Choose whether to print your personal blob sheet before the next user starts.</p>

            <div class="actions">
                <Button variant="secondary" @click="emit('skip-printing')">
                    Skip Printing
                </Button>
                <Button variant="primary" @click="emit('print-sheet')">
